...
Public Class SignalGeneratorProcedure
    ...
    Public Sub New(DUTAssetNumber As String)
    ...
    Private Sub InitializeInstruments()
        If (Me.DUT Is Nothing) Then
            Select Case Me.SelectedDUTSignalGenerator
                Case "AgilentE4433B"
        ...
        If (Me.LowFrequencySigGen Is Nothing) Then
            Select Case Me.SelectedSignalGenerator
                Case "AgilentE4433B"
        ...
        If (Me.SpecAn Is Nothing) Then
            Select Case Me.SelectedSpectrumAnalyzer
                Case "Agilent8564EC"
    ...
    Public Sub AttenuatorAccuracyTest()
        ...
        Try
            ...
            For Each TestFrequency As Double In Me.DUT.TestPoints.AttenuatorAccuracyTestFrequencies
                ...
                Me.MeasuringRec.Write("22.32SP")
                For CurrentAmplitude As Integer = 0 To Convert.ToInt32(Me.MeasuringRec.Sections.Measurements.GetRoundedTunedRFAmplitude(Me.DUT.Capabilities.MinimumRFAmplitude)) Step -10
                    Me.MeasuringRec.Sections.Triggers.Immediate()
                    Dim SPR As SerialPollFlags = SerialPollFlags.RequestingService
                    Try
                        SPR = Me.MeasuringRec.SerialPoll
                    Catch Ex As Exception
                    End Try
                    'TODO:  This was "SPR"; Changed to SPR = SerialPollFlags.MessageAvailable, but unsure if that's correct.
                    If SPR = SerialPollFlags.MessageAvailable Then
    ...
    Public Sub FMCarrierFrequencyAccuracyTest()
        ...
        Dim FMReadingArray() As String = FMReading.Replace(ControlChars.Cr, "").Split(ControlChars.Lf)
        ...
        FMReadingArray = FMReading.Replace(ControlChars.Cr, "").Split(ControlChars.Lf)
    ...
    Public Sub FrequencyAccuracyTest()
        ...
        Dim TempDataArray() As String = TempData.Replace(ControlChars.Cr, "").Split(ControlChars.Lf)
